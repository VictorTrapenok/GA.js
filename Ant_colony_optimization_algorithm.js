/*
 * Ant_colony_optimization_algorithm.js
 *
...
Ant_colony_optimization_algorithm = function ()
{
    ...
    {
        //console.log("ReGenFrom",  start)
        if( start === undefined || isNaN(start) || start > base_entity.length -3 || start < 1 )
        {
            return base_entity;
        }
    ...
    this.getAnswer = function()
    {
        ...
        do{
            ...
            for(var j=0; j <  this.generation_size-1 ; j++ ) // Генерация похожих путей с положительной обратной связью
            {
                for(var i=0; i < this.generation_size - j  ; i ) // Генерация похожих путей из лучшей половины
                {
                    var random_start = Math.floor( Math.random()*this.generation[i].length ) +1 
                    var new_entity = this.ReGenFrom(this.generation[i], random_start); // Изменение цепочки начиная с случайного элемента и до конца
                ...
                temp_generation_array.sort(function(a, b){ // Сортировка сгенерированых поколений по степени их пригодности
                    return a.length  - b.length;
                })
            }
            this.generation = temp_generation_array.slice(0, this.generation_size);
    ...
    var sa = new Ant_colony_optimization_algorithm()
    answer = sa.getAnswer()
